...
# FoNE SFT: Llama 1.5B on GSM8K with 2 GPUs (Interactive)
# Usage: bash scripts/sft_1p5b_2gpu.sh /path/to/pretrained/model
...
if [ $# -eq 0 ]; then
    ...
    echo "Usage: bash scripts/sft_1p5b_2gpu.sh /path/to/pretrained/model"
    exit 1
fi
